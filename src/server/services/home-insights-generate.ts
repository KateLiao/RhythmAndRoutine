...
要求：
- 必须基于事实，不得编造日程
- 每条建议必须有 proposedChange（改期/新增/打开表单/打开执行记录 open_execution_feedback）
- 计划时段已过但 status=planned 时，优先建议 open_execution_feedback，不要建议改到此刻开始
- 主建议应对应当前最合适的单一行动
- 备选应覆盖不同合理策略，但不要重复
